...
- アイテムカードをクリックするとそのアイテムをエディタにロード（編集開始）
- **編集中のアイテムを再クリックするとキャンセル**（`handleClear()` を呼び出してフォームをリセット）

#### レイアウト

- 3カラム構成（左:フォーム, 中央:グリッド, 右:アイテム一覧）を `justify-center items-start` で水平中央寄せ
- グリッドとアイテム一覧の間に `ml-8` の追加マージン
- アイテム一覧の幅: `w-72`（288px）
